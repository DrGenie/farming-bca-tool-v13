...
(function () {
  ...
  function setBusy(b) {
    ...
    e.quick.querySelectorAll('button').forEach((btn) => btn.disabled = b);
  }
  ...
  function updateBadges() {
    ...
    const has = !!(ts && ts.hasResults);
    setBadge(e.badgeCurrent, has ? 'on' : 'off', has ? 'Using current result' : 'Run the analysis first');
    const n = ts ? (ts.treatmentsRanked || 0) : 0;
    setBadge(e.badgeSaved, n > 0 ? 'on' : 'off', n > 0 ? (n + ' treatment' + (n === 1 ? '' : 's') + ' ranked') : 'No treatments ranked');
  }
  async function checkHealth() {
    const e = els();
    if (!configured()) { setBadge(e.badgeBackend, 'off', 'Backend not set'); return; }
    setBadge(e.badgeBackend, 'checking', 'Checking backend');
    try {
  ...
  async function send(question, actionType, displayText) {
    ...
    if (RESULT_ACTIONS.has(actionType)) {
      const ts = toolState();
      if (!ts || !ts.hasResults) {
        addMessage('assistant', NEEDS_ANALYSIS_MSG, { noActions: true });
        return;
      }
    }
